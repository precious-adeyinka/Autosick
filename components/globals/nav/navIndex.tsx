  ...
  return (
    <nav className='h-20 bg-transparent w-full flex items-center justify-between'>
      ...
      <div className='flex items-center justify-end space-x-5'>
        {isLoading ? <div className='bg-gray-100 rounded-full px-5 py-2 text-black flex items-center justify-center'>
          Loading...
        </div> : !user ? <a href='/api/auth/login' className='bg-gray-100 rounded-full px-5 py-2 text-black flex items-center justify-center'>
          Sign In
        </a> : <a href='/profile'>
          <img className='w-[45px] aspect-square rounded-full' src={user.picture as string} />
        </a>}
        {/* hamburger */}
